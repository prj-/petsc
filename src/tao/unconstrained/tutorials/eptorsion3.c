...
int main(int argc, char **argv)
{
  ...
  PetscCall(VecDuplicate(user.y, &lb));
  PetscCall(VecDuplicate(user.y, &ub));

  PetscCall(VecSet(ub, 0.1));
  PetscCall(VecSet(lb, -0.1));
  PetscCall(VecSetValue(ub, i, 0., INSERT_VALUES));
  PetscCall(VecSetValue(lb, i, 0., INSERT_VALUES));
  PetscCall(VecAssemblyBegin(ub));
  PetscCall(VecAssemblyBegin(lb));
  PetscCall(VecAssemblyEnd(ub));
  PetscCall(VecAssemblyEnd(lb));
