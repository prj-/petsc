...
PetscErrorCode DMView_Network(DM dm, PetscViewer viewer)
{
  ...
  if (isascii) {
    ...
    PetscInt vfrom, vto, nv, ne, nsv, p, nsubnet;
    DM_Network     *network = (DM_Network *)dm->data;
    PetscMPIInt     rank;
    ...
    if (nsv) {
      ...
      for (PetscInt i = 0; i < nsv; i++) {
        ...
        for (PetscInt j = 1; j < nv; j++) PetscCall(PetscViewerASCIISynchronizedPrintf(viewer, "                                           ----> subnet[%" PetscInt_FMT "].%" PetscInt_FMT "\n", sv[2 * j], sv[2 * j + 1]));
      }
    }
    PetscCall(PetscViewerFlush(viewer));
    PetscCall(PetscViewerASCIIPopSynchronized(viewer));
