    ...
    for (i = si; i < si + nx; i++) {
      ...
      for (p = 0; p < GAUSS_POINTS; p++) {
        ...
        PetscCall(PetscFPrintf(PETSC_COMM_SELF, fp, "%1.6e %1.6e %1.6e %1.6e\n", (double)PetscRealPart(_coefficients[j][i].E[p]), (double)PetscRealPart(_coefficients[j][i].nu[p]), (double)PetscRealPart(_coefficients[j][i].fx[p]),
                               (double)PetscRealPart(_coefficients[j][i].fy[p])));
      }
    }
  }
...
static void FormStressOperatorQ1(PetscScalar Ke[], PetscScalar coords[], PetscScalar E[], PetscScalar nu[])
{
  ...
  for (p = 0; p < ngp; p++) {
    ...
    constit_D[0][0] = 1.0 - prop_nu;
    constit_D[0][1] = prop_nu;
    constit_D[0][2] = 0.0;
    constit_D[1][0] = prop_nu;
    constit_D[1][1] = 1.0 - prop_nu;
    constit_D[1][2] = 0.0;
    constit_D[2][0] = 0.0;
    constit_D[2][1] = 0.0;
...
static PetscErrorCode GetElementCoords(DMDACoor2d **_coords, PetscInt ei, PetscInt ej, PetscScalar el_coords[])
{
  ...
  el_coords[NSD * 1 + 0] = _coords[ej + 1][ei].x;
  el_coords[NSD * 1 + 1] = _coords[ej + 1][ei].y;
  el_coords[NSD * 2 + 0] = _coords[ej + 1][ei + 1].x;
