...
int main(int argc, char **argv)
{
  ...
  if (flg) {
    ...
    PetscCheck(norm <= PETSC_SMALL, PetscObjectComm((PetscObject)A), PETSC_ERR_PLIB, "Difference of both solves has nonzero norm, ||x - y||_inf = %g > %g", (double)norm, (double)PETSC_SMALL);
