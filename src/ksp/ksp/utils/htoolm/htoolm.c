...
{
  PetscFunctionBegin;
  PetscCall(PetscObjectCompose((PetscObject)F, "ksp", NULL));
...
static PetscErrorCode MatFactorSymbolic_NestHtool_FieldSplit(Mat F, Mat A)
{
  ...
  for (PetscInt i = 0; i < nr; ++i) {
    ...
    PetscCheck(same, PetscObjectComm((PetscObject)A), PETSC_ERR_SUP, "Only congruent MATNEST row/column maps are supported");
  }
  for (PetscInt i = 0; i < nr; ++i) {
    PetscBool ishtool, isdense;
    PetscCall(PetscObjectTypeCompare((PetscObject)mats[i][i], MATHTOOL, &ishtool));
    PetscCall(PetscObjectTypeCompareAny((PetscObject)mats[i][i], &isdense, MATSEQDENSE, MATMPIDENSE, NULL));
    if (ishtool) {
      PetscCheck(!found, PetscObjectComm((PetscObject)A), PETSC_ERR_SUP, "Exactly one diagonal MATHTOOL block is required");
      found = PETSC_TRUE;
      hidx  = i;
    } else PetscCheck(isdense, PetscObjectComm((PetscObject)A), PETSC_ERR_SUP, "Diagonal block %" PetscInt_FMT " must be MATHTOOL or dense", i);
  }
  ...
  PetscCall(PetscObjectCompose((PetscObject)F, "ksp", (PetscObject)ksp));
...
static PetscErrorCode MatFactorNumeric_NestHtool(Mat F, Mat A, MatFactorType factor_type)
{
  ...
  PetscCall(PetscObjectQuery((PetscObject)F, "ksp", (PetscObject *)&ksp));
  PetscCheck(ksp, PetscObjectComm((PetscObject)A), PETSC_ERR_ARG_WRONGSTATE, "Outer fieldsplit has not been created yet, call symbolic factorization first");

  PetscCall(KSPSetOperators(ksp, A, A));
  PetscCall(KSPSetUp(ksp));
  PetscCall(KSPGetPC(ksp, &pc));
  PetscCall(PCFieldSplitGetSubKSP(pc, &nsplits, &subksp));
  PetscCheck(nsplits == 1 || nsplits == 2, PetscObjectComm((PetscObject)A), PETSC_ERR_PLIB, "Unexpected number of field splits %" PetscInt_FMT, nsplits);
  for (PetscInt i = 0; i < nsplits; ++i) {
    PC subpc;

    PetscCall(KSPSetType(subksp[i], KSPPREONLY));
    PetscCall(KSPGetPC(subksp[i], &subpc));
    PetscCall(PCSetType(subpc, factor_type == MAT_FACTOR_LU ? PCLU : PCCHOLESKY));
  }
