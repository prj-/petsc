...
static PetscErrorCode MatFactorNumeric_NestHtool(Mat F, Mat A, MatFactorType factor_type)
{
  ...
  for (PetscInt i = 0; i < nsplits; ++i) {
    Mat       subA;
    PC        subpc;
    PetscBool ishtool, issymhtool = PETSC_FALSE;

    PetscCall(KSPSetType(subksp[i], KSPPREONLY));
    PetscCall(KSPGetPC(subksp[i], &subpc));
    PetscCall(KSPGetOperators(subksp[i], &subA, NULL));
    if (subA) PetscCall(PetscObjectTypeCompare((PetscObject)subA, MATHTOOL, &ishtool));
    else ishtool = PETSC_FALSE;
    if (ishtool) issymhtool = (PetscBool)(subA->symmetric == PETSC_BOOL3_TRUE || subA->hermitian == PETSC_BOOL3_TRUE);
    PetscCall(PCSetType(subpc, factor_type == MAT_FACTOR_CHOLESKY || issymhtool ? PCCHOLESKY : PCLU));
  }
