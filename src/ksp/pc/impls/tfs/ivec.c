...
PetscErrorCode PCTFS_ivec_sort(PetscInt *ar, PetscInt size)
{
  ...
  for (;;) {
    ...
    if (size > SORT_OPT) {
      ...
      else { /* might as well pop - note NR_OPT >=2 ==> we're ok! */ ar = *(--top_a);
        size                                                            = *(--top_s);
      }
      ...
      if (top_s == bottom_s) PetscFunctionReturn(PETSC_SUCCESS);
    ...
    if (size > SORT_OPT) {
      ...
      if ((*top_s = size - ((PetscInt)(pi - ar)))) {
        ...
        *(top_a++) = pi2;
        ...
        ; /* set up for next loop iff there is something to do */
      else { /* might as well pop - note NR_OPT >=2 ==> we're ok! */ ar2 = *(--top_a);
        ar                                                               = *(--top_a);
        size                                                             = *(--top_s);
      }
    } else { /* else sort small list directly then pop another off stack */
    ...
    if (size > SORT_OPT) {
      ...
      if ((*top_s = size - ((PetscInt)(pi - ar)))) {
        ...
        *(top_a++) = (PetscInt *)pi2;
        ...
        ; /* set up for next loop iff there is something to do */
      else { /* might as well pop - note NR_OPT >=2 ==> we're ok! */ ar2 = (PetscInt **)*(--top_a);
        ar                                                               = *(--top_a);
        size                                                             = *(--top_s);
      }
    } else { /* else sort small list directly then pop another off stack */
...
/*********************************ivec.c*************************************/
static PetscErrorCode PCTFS_rvec_exists(PetscScalar *arg1, PetscScalar *arg2, PetscInt n)
{
  PetscFunctionBegin;
  while (n--) {
