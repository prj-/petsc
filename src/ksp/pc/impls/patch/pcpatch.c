...
@*/
PetscErrorCode PCPatchSetComputeFunction(PC pc, PetscErrorCode (*func)(PC pc, PetscInt point, Vec x, Vec f, IS cellIS, PetscInt n, const PetscInt *dofsArray, const PetscInt *dofsArrayWithAll, PetscCtx ctx), PetscCtx ctx)
{
  PC_PATCH *patch = (PC_PATCH *)pc->data;
...
@*/
PetscErrorCode PCPatchSetComputeFunctionInteriorFacets(PC pc, PetscErrorCode (*func)(PC pc, PetscInt point, Vec x, Vec f, IS facetIS, PetscInt n, const PetscInt *dofsArray, const PetscInt *dofsArrayWithAll, PetscCtx ctx), PetscCtx ctx)
{
  PC_PATCH *patch = (PC_PATCH *)pc->data;
...
@*/
PetscErrorCode PCPatchSetComputeOperator(PC pc, PetscErrorCode (*func)(PC pc, PetscInt point, Vec x, Mat mat, IS facetIS, PetscInt n, const PetscInt *dofsArray, const PetscInt *dofsArrayWithAll, PetscCtx ctx), PetscCtx ctx)
{
  PC_PATCH *patch = (PC_PATCH *)pc->data;
...
.seealso: [](ch_ksp), `PCPatchGetComputeOperator()`, `PCPatchSetComputeFunction()`, `PCPatchSetDiscretisationInfo()`
@*/
PetscErrorCode PCPatchSetComputeOperatorInteriorFacets(PC pc, PetscErrorCode (*func)(PC pc, PetscInt point, Vec x, Mat mat, IS facetIS, PetscInt n, const PetscInt *dofsArray, const PetscInt *dofsArrayWithAll, PetscCtx ctx), PetscCtx ctx)
{
  PC_PATCH *patch = (PC_PATCH *)pc->data;
...
static PetscErrorCode PCPatchCompleteCellPatch(PC pc, PetscHSetI ht, PetscHSetI cht)
{
  ...
  PetscInt fBegin, fEnd, fStarSize, fClosureSize;
      ...
      for (ci = 0; ci < closureSize * 2; ci += 2) {
        ...
        if (patch->usercomputeopintfacet) {
          if (fBegin <= seenpoint && seenpoint < fEnd) {
            ...
            for (PetscInt fsi = 0; fsi < fStarSize * 2; fsi += 2) {
              PetscCall(DMPlexGetTransitiveClosure(dm, fStar[fsi], PETSC_TRUE, &fClosureSize, &fClosure));
              for (PetscInt fci = 0; fci < fClosureSize * 2; fci += 2) PetscCall(PetscHSetIAdd(cht, fClosure[fci]));
...
static PetscErrorCode PCPatchCreateCellPatchDiscretisationInfo(PC pc)
{
  ...
  if (patch->combined) {
    ...
    PetscInt pStartf, pEndf;
    pStart = PETSC_INT_MAX;
    pEnd   = PETSC_INT_MIN;
    for (PetscInt f = 0; f < patch->nsubspaces; ++f) {
    ...
    if (rsize < 3000) { /* FIXME: I picked this switch value out of my hat */
      ...
      if (patch->usercomputeopintfacet) {
        ...
        for (i = 0; i < numIntFacets; i++) {
          ...
          for (PetscInt celli = 0; celli < patch->totalDofsPerCell; celli++) {
            const PetscInt row = dofsArray[(offset + cell0) * patch->totalDofsPerCell + celli];
            if (row < 0) continue;
            for (PetscInt cellj = 0; cellj < patch->totalDofsPerCell; cellj++) {
...
static PetscErrorCode PCPatchComputeFunction_DMPlex_Private(PC pc, PetscInt patchNum, Vec x, Vec F, IS cellIS, PetscInt n, const PetscInt *l2p, const PetscInt *l2pWithAll, PetscCtx ctx)
{
  PC_PATCH       *patch = (PC_PATCH *)pc->data;
  DM              dm, plex;
  PetscSection    s;
...
static PetscErrorCode PCPatchComputeOperator_DMPlex_Private(PC pc, PetscInt patchNum, Vec x, Mat J, IS cellIS, PetscInt n, const PetscInt *l2p, const PetscInt *l2pWithAll, PetscCtx ctx)
{
  PC_PATCH       *patch = (PC_PATCH *)pc->data;
  DM              dm, plex;
  PetscSection    s;
...
static PetscErrorCode PCPatchPrecomputePatchTensors_Private(PC pc)
{
  ...
  if (!patch->allCells) {
    ...
    for (i = pStart; i < pEnd; i++) {
      ...
      if (ncell <= 0) continue;
      for (PetscInt j = 0; j < ncell; j++) PetscCall(PetscHSetIAdd(cells, cellsArray[offset + j]));
    }
    PetscCall(ISRestoreIndices(patch->cells, &cellsArray));
  ...
  PetscCall(MatShellSetOperation(vecMat, MATOP_SET_VALUES, (PetscErrorCodeFn *)MatSetValues_PCPatch_Private));
  PetscCall(ISGetSize(patch->allCells, &ncell));
  ...
  if (patch->usercomputeopintfacet) {
    ...
    if (!patch->allIntFacets) {
      ...
      for (i = pStart; i < pEnd; i++) {
        ...
        for (PetscInt j = 0; j < nIntFacets; j++) PetscCall(PetscHSetIAdd(facets, intFacetsArray[offset + j]));
      }
      PetscCall(ISRestoreIndices(patch->intFacets, &intFacetsArray));
...
PetscErrorCode PCPatch_ScatterLocal_Private(PC pc, PetscInt p, Vec x, Vec y, InsertMode mode, ScatterMode scat, PatchScatterType scattertype)
{
  ...
  for (PetscInt lidx = 0; lidx < dof; ++lidx) {
  ...
  if (patch->save_operators) {
    ...
    for (PetscInt i = 0; i < patch->npatch; ++i) {
      ...
      if (!patch->denseinverse) {
        ...
        PetscCall(MatGetOperation(patch->mat[i], MATOP_MULT, (PetscErrorCodeFn **)&patch->densesolve));
      }
    }
  }
  if (patch->local_composition_type == PC_COMPOSITE_MULTIPLICATIVE) {
  ...
  if (!pc->setupcalled) {
    ...
    if (!patch->nsubspaces) {
      ...
      for (p = pStart; p < pEnd; ++p) {
        ...
        for (PetscInt d = 0; d < cdof; ++d) globalBcs[numGlobalBcs++] = off + ind[d];
      }
    ...
    for (p = pStart; p < pEnd; ++p) {
      if (isNonlinear) {
        ...
        for (PetscInt i = 0; i < numPatchDofsWithAll; i++) {
    ...
    if (patch->save_operators) {
      ...
      for (PetscInt i = 0; i < patch->npatch; ++i) PetscCall(PCPatchCreateMatrix_Private(pc, i, &patch->mat[i], PETSC_FALSE));
    }
...
static PetscErrorCode PCApply_PATCH(PC pc, Vec x, Vec y)
{
  ...
  PetscInt pStart, numBcs, n, sweep;
  ...
  for (PetscInt bc = 0; bc < numBcs; ++bc) {
...
static PetscErrorCode PCReset_PATCH(PC pc)
{
  ...
  if (patch->patch_dof_weights) {
    for (PetscInt i = 0; i < patch->npatch; ++i) PetscCall(VecDestroy(&patch->patch_dof_weights[i]));
    PetscCall(PetscFree(patch->patch_dof_weights));
  }
  if (patch->mat) {
    for (PetscInt i = 0; i < patch->npatch; ++i) PetscCall(MatDestroy(&patch->mat[i]));
    PetscCall(PetscFree(patch->mat));
  }
  if (patch->matWithArtificial && !patch->isNonlinear) {
    for (PetscInt i = 0; i < patch->npatch; ++i) PetscCall(MatDestroy(&patch->matWithArtificial[i]));
  ...
  if (patch->dofMappingWithoutToWithArtificial) {
    for (PetscInt i = 0; i < patch->npatch; ++i) PetscCall(ISDestroy(&patch->dofMappingWithoutToWithArtificial[i]));
    PetscCall(PetscFree(patch->dofMappingWithoutToWithArtificial));
  }
  if (patch->dofMappingWithoutToWithAll) {
    for (PetscInt i = 0; i < patch->npatch; ++i) PetscCall(ISDestroy(&patch->dofMappingWithoutToWithAll[i]));
    PetscCall(PetscFree(patch->dofMappingWithoutToWithAll));
  }
  PetscCall(PetscFree(patch->sub_mat_type));
  if (patch->userIS) {
    for (PetscInt i = 0; i < patch->npatch; ++i) PetscCall(ISDestroy(&patch->userIS[i]));
...
static PetscErrorCode PCDestroy_PATCH_Linear(PC pc)
{
  ...
  PetscFunctionBegin;
  if (patch->solver) {
    for (PetscInt i = 0; i < patch->npatch; ++i) PetscCall(KSPDestroy((KSP *)&patch->solver[i]));
