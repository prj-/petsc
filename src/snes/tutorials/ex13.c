...
static PetscErrorCode ComputeSpectral(Vec u, PetscInt numPlanes, const PetscInt planeDir[], const PetscReal planeCoord[], AppCtx *user)
{
  ...
  PetscCall(DMGetCoordinateSection(dm, &coordSection));
  PetscCall(VecGetArrayRead(coordinates, &coords));
  for (PetscInt p = 0; p < numPlanes; ++p) {
    DMLabel         label;
    char            name[PETSC_MAX_PATH_LEN];
    Mat             F;
    Vec             x, y;
...
static PetscErrorCode ComputeAdjoint(Vec u, AppCtx *user)
{
  ...
  /* Error representation */
  {
    ...
    for (PetscInt i = 0; i < 2; ++i) PetscCall(ISDestroy(&subis[i]));
  ...
  test:
    ...
    args: -potential_petscspace_degree 1 -dm_plex_box_faces 2,2 -dm_refine_hierarchy 3 \
          -ksp_rtol 5e-10 -pc_type mg \
            ...
            -mg_levels_pc_type jacobi
  # Run with -dm_refine_hierarchy 3 to get a better idea of the solver
  testset:
    args: -potential_petscspace_degree 1 -dm_refine_hierarchy 2 \
    ...
    test:
      ...
      requires: ctetgen
      args: -dm_plex_dim 3 -dm_plex_box_faces 2,2,1
    test:
      suffix: 3d_q1_gmg_fcycle
      args: -dm_plex_dim 3 -dm_plex_simplex 0 -dm_plex_box_faces 2,2,1
  test:
