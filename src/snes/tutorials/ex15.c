...
int main(int argc, char **argv)
{
  ...
  if (user.picard) {
    /*
        ...
        the SNES to set it
    */
    PetscCall(DMDASNESSetPicardLocal(da, INSERT_VALUES, (PetscErrorCode (*)(DMDALocalInfo *, void *, void *, PetscCtx))FormFunctionPicardLocal, (PetscErrorCode (*)(DMDALocalInfo *, void *, Mat, Mat, PetscCtx))FormJacobianLocal, &user));
      ...
      if (i == 0 || j == 0 || i == Mx - 1 || j == My - 1) {
        ...
        if (user->initial == -1) {
          if (user->lambda != 0) {
            ...
            const PetscReal xx = 2 * (PetscReal)i / (Mx - 1) - 1, yy = 2 * (PetscReal)j / (My - 1) - 1;
...
/*TEST
   ...
   test:
      suffix: 5
      args: -snes_monitor_short -snes_type newtontr -npc_snes_type ngs -snes_npc_side right -da_grid_x 20 -da_grid_y 20 -p 1.3 -lambda 1 -ksp_converged_reason -pc_type ilu
      requires: !single
