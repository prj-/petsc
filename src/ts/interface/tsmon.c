...
PetscErrorCode TSMonitorSetFromOptions(TS ts, const char name[], const char help[], const char manual[], PetscErrorCode (*monitor)(TS ts, PetscInt step, PetscReal time, Vec u, PetscViewerAndFormat *vf), PetscErrorCode (*monitorsetup)(TS ts, PetscViewerAndFormat *vf))
{
  ...
  if (flg) {
    ...
    PetscCall(TSMonitorSet(ts, (PetscErrorCode (*)(TS, PetscInt, PetscReal, Vec, PetscCtx))monitor, vf, (PetscCtxDestroyFn *)PetscViewerAndFormatDestroy));
  }
  PetscFunctionReturn(PETSC_SUCCESS);
}
...
{
  TSMonitorDrawCtx ictx = (TSMonitorDrawCtx)ctx;
  PetscDraw        draw;
...
{
  TSMonitorDrawCtx ctx    = (TSMonitorDrawCtx)Ctx;
...
{
  TSMonitorLGCtx     ctx = (TSMonitorLGCtx)Ctx;
  const PetscScalar *yy;
  Vec                y;
