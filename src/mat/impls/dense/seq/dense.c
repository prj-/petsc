  ...
  while (its--) {
    if (flag & SOR_FORWARD_SWEEP || flag & SOR_LOCAL_FORWARD_SWEEP) {
      for (i = 0; i < m; i++) {
        ...
        x[i] = (1. - omega) * x[i] + (xt + v[i + i * m] * x[i]) * omega / (v[i + i * m] + shift);
      }
    }
...
PETSC_INTERN PetscErrorCode MatMultColumnRangeKernel_SeqDense(Mat A, Vec xx, Vec yy, PetscInt c_start, PetscInt c_end, PetscBool trans, PetscBool herm)
{
  Mat_SeqDense      *mat = (Mat_SeqDense *)A->data;
...
PETSC_INTERN PetscErrorCode MatMultAddColumnRangeKernel_SeqDense(Mat A, Vec xx, Vec zz, Vec yy, PetscInt c_start, PetscInt c_end, PetscBool trans, PetscBool herm)
{
  Mat_SeqDense      *mat = (Mat_SeqDense *)A->data;
  const PetscScalar *v   = mat->v, *x;
...
{
  PetscFunctionBegin;
  PetscCall(MatMultColumnRangeKernel_SeqDense(A, xx, yy, c_start, c_end, PETSC_FALSE, PETSC_FALSE));
  PetscFunctionReturn(PETSC_SUCCESS);
}
...
PetscErrorCode MatNorm_SeqDense(Mat A, NormType type, PetscReal *nrm)
{
  ...
  if (type == NORM_FROBENIUS) {
    ...
    *nrm = 0.0;
    ...
    PetscReal   *s;
    PetscBLASInt bm, bn, blda, min, lwork, info;
...
static struct _MatOps MatOps_Values = {MatSetValues_SeqDense,
                                       ...
                                       /* 89*/ NULL,
                                       MatProductSetFromOptions_SeqDense,
                                       NULL,
                                       NULL,
                                       ...
                                       NULL,
                                       NULL,
                                       /*129*/ MatCreateMPIMatConcatenateSeqMat_SeqDense,
                                       NULL,
                                       NULL,
                                       NULL,
                                       NULL,
...
PetscErrorCode MatDenseRestoreColumnVec(Mat A, PetscInt col, Vec *v)
{
  ...
  PetscValidType(A, 1);
  PetscValidLogicalCollectiveInt(A, col, 2);
  if (v && *v) PetscValidHeaderSpecific(*v, VEC_CLASSID, 3);
  PetscCheck(A->preallocated, PetscObjectComm((PetscObject)A), PETSC_ERR_ORDER, "Matrix not preallocated");
  PetscCheck(col >= 0 && col < A->cmap->N, PetscObjectComm((PetscObject)A), PETSC_ERR_ARG_WRONG, "Invalid col %" PetscInt_FMT ", should be in [0,%" PetscInt_FMT ")", col, A->cmap->N);
