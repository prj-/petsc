...
static PetscErrorCode MatCreateSubMatrices_Nest(Mat A, PetscInt n, const IS irow[], const IS icol[], MatReuse scall, Mat *submat[])
{
  ...
  PetscInt         bi, bj, k, r, c;
  const PetscInt **row_ranges, **col_ranges;

  PetscFunctionBegin;
  PetscCall(PetscMalloc1(nr, &row_ranges));
  PetscCall(PetscMalloc1(nc, &col_ranges));
  for (bi = 0; bi < nr; bi++) {
    Mat sub;

    PetscCall(MatNestFindNonzeroSubMatRow(A, bi, &sub));
    PetscCheck(sub, PetscObjectComm((PetscObject)A), PETSC_ERR_ARG_WRONG, "Row block %" PetscInt_FMT " has no nonzero submatrix", bi);
    PetscCall(MatGetOwnershipRanges(sub, &row_ranges[bi]));
  }
  for (bj = 0; bj < nc; bj++) {
    Mat sub;

    PetscCall(MatNestFindNonzeroSubMatCol(A, bj, &sub));
    PetscCheck(sub, PetscObjectComm((PetscObject)A), PETSC_ERR_ARG_WRONG, "Column block %" PetscInt_FMT " has no nonzero submatrix", bj);
    PetscCall(MatGetOwnershipRangesColumn(sub, &col_ranges[bj]));
  }
  if (scall == MAT_INITIAL_MATRIX) PetscCall(PetscCalloc1(n, submat));
  for (k = 0; k < n; k++) {
    ...
    for (r = 0; r < nrow_k; r++) {
      ...
      PetscCall(PetscLayoutFindOwner(A->rmap, g, &p));
      local  = g - A->rmap->range[p];
      cumoff = 0;
      for (bi = 0; bi < nr; bi++) {
        PetscInt nb_p = row_ranges[bi][p + 1] - row_ranges[bi][p];

        if (local >= cumoff && local < cumoff + nb_p) {
          row_block[r] = bi;
          row_bglob[r] = row_ranges[bi][p] + (local - cumoff);
          break;
        }
        cumoff += nb_p;
      }
    ...
    for (c = 0; c < ncol_k; c++) {
      PetscInt    g = col_idx[c], local, cumoff;
      PetscMPIInt p;

      PetscCall(PetscLayoutFindOwner(A->cmap, g, &p));
      local  = g - A->cmap->range[p];
      cumoff = 0;
      for (bj = 0; bj < nc; bj++) {
        PetscInt nb_p = col_ranges[bj][p + 1] - col_ranges[bj][p];

        if (local >= cumoff && local < cumoff + nb_p) {
          col_block[c] = bj;
          col_bglob[c] = col_ranges[bj][p] + (local - cumoff);
          break;
        }
        cumoff += nb_p;
      }
...
static PetscErrorCode MatNestSetSubMat_Nest(Mat A, PetscInt idxm, PetscInt jdxm, Mat mat)
{
  ...
  if (mat) PetscCall(MatGetNonzeroState(mat, &bA->nnzstate[idxm * bA->nc + jdxm]));
  else bA->nnzstate[idxm * bA->nc + jdxm] = 0;
  A->nonzerostate++;
  PetscFunctionReturn(PETSC_SUCCESS);
}
...
PetscErrorCode MatNestSetSubMat(Mat A, PetscInt idxm, PetscInt jdxm, Mat sub)
{
  ...
  PetscValidLogicalCollectiveInt(A, idxm, 2);
  PetscValidLogicalCollectiveInt(A, jdxm, 3);
  if (sub) PetscValidHeaderSpecific(sub, MAT_CLASSID, 4);
  PetscTryMethod(A, "MatNestSetSubMat_C", (Mat, PetscInt, PetscInt, Mat), (A, idxm, jdxm, sub));
  PetscFunctionReturn(PETSC_SUCCESS);
}
...
static PetscErrorCode MatNestSetSubMats_Nest(Mat A, PetscInt nr, const IS is_row[], PetscInt nc, const IS is_col[], const Mat a[])
{
  ...
  for (i = 0; i < nr; i++) {
    s->m[i] = s->m[0] + i * nc;
    for (j = 0; j < nc; j++) {
      s->m[i][j] = a ? a[i * nc + j] : NULL;
      PetscCall(PetscObjectReference((PetscObject)s->m[i][j]));
    }
  }
      ...
      for (j = 0; j < nc; j++) {
        if (s->m[i][j]) {
          if (!vtype) { /* first visited block */
            PetscCall(MatGetVecType(s->m[i][j], &vtype));
            sametype = PETSC_TRUE;
          } else if (sametype) {
            PetscCall(MatGetVecType(s->m[i][j], &type));
...
PetscErrorCode MatNestSetSubMats(Mat A, PetscInt nr, const IS is_row[], PetscInt nc, const IS is_col[], const Mat a[]) PeNSS
{
  PetscFunctionBegin;
  PetscValidHeaderSpecific(A, MAT_CLASSID, 1);
  PetscValidLogicalCollectiveInt(A, nr, 2);
  ...
  if (nr && is_row) {
    ...
    for (PetscInt i = 0; i < nr; i++) PetscValidHeaderSpecific(is_row[i], IS_CLASSID, 3);
  }
  PetscValidLogicalCollectiveInt(A, nc, 4);
  ...
  if (nc && is_col) {
    ...
    for (PetscInt i = 0; i < nc; i++) PetscValidHeaderSpecific(is_col[i], IS_CLASSID, 5);
  }
  PetscTryMethod(A, "MatNestSetSubMats_C", (Mat, PetscInt, const IS[], PetscInt, const IS[], const Mat[]), (A, nr, is_row, nc, is_col, a));
  PetscFunctionReturn(PETSC_SUCCESS);
}
...
PetscErrorCode MatCreateNest(MPI_Comm comm, PetscInt nr, const IS is_row[], PetscInt nc, const IS is_col[], const Mat a[], Mat *B) PeNSS
{
  ...
  PetscCall(MatSetType(*B, MATNEST));
  (*B)->preallocated = PETSC_TRUE;
  PetscCall(MatNestSetSubMats(*B, nr, is_row, nc, is_col, a));
  PetscFunctionReturn(PETSC_SUCCESS);
}
