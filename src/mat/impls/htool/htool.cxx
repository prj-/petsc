...
static PetscErrorCode MatMult_Htool(Mat A, Vec x, Vec y)
{
  ...
  if (a->permutation) htool::add_distributed_operator_vector_product_local_to_local<PetscScalar>('N', 1.0, *a->distributed_operator, in, 0.0, out, nullptr);
...
static PetscErrorCode MatMultTranspose_Htool(Mat A, Vec x, Vec y)
{
  ...
  PetscCall(VecGetArrayRead(x, &in));
  PetscCall(VecGetArrayWrite(y, &out));
  if (a->permutation) htool::add_distributed_operator_vector_product_local_to_local<PetscScalar>('T', 1.0, *a->distributed_operator, in, 0.0, out, nullptr);
...
static PetscErrorCode MatView_Htool_Draw_Zoom(PetscDraw draw, void *ptr)
{
  ...
  for (const auto *block : low_rank_blocks) {
...
static PetscErrorCode MatProductNumeric_Htool(Mat C)
{
  ...
  switch (product->type) {
  case MATPRODUCT_AB:
    if (a->permutation) htool::add_distributed_operator_matrix_product_local_to_local<PetscScalar>('N', 1.0, *a->distributed_operator, in, 0.0, out, N, nullptr);
    ...
    break;
  case MATPRODUCT_AtB:
    if (a->permutation) htool::add_distributed_operator_matrix_product_local_to_local<PetscScalar>('T', 1.0, *a->distributed_operator, in, 0.0, out, N, nullptr);
