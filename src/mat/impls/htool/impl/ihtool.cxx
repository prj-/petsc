...
static PetscErrorCode MatGetDiagonalBlock_Htool(Mat A, Mat *b)
{
  ...
  if (B) {
    ...
    if (PetscAbsScalar(scale[0] - scale[1]) > PETSC_MACHINE_EPSILON || PetscAbsScalar(shift[0] - shift[1]) > PETSC_MACHINE_EPSILON) B = nullptr;
  }
  if (!B) {
...
static PetscErrorCode MatFactorCtxDestroy(PetscCtx ctx)
{
  MatFactorCtx **data = (MatFactorCtx **)ctx;

  PetscFunctionBegin;
  /* PetscContainer invokes PetscCtxDestroyFn() with &container->ctx, i.e., pointer-to-stored-pointer */
  delete (*data)->hmatrix;
  /* PetscContainer invokes PetscCtxDestroyFn() with &container->ctx, i.e., pointer-to-stored-pointer */
...
static inline PetscErrorCode MatSolve_Private(Mat A, htool::Matrix<PetscScalar> &X)
{
  ...
  PetscCheck(data->hmatrix, PETSC_COMM_SELF, PETSC_ERR_ARG_WRONGSTATE, "Must call Mat%sFactorNumeric() before Mat%sSolve%s()", A->factortype == MAT_FACTOR_LU ? "LU" : "Cholesky", X.nb_cols() == 1 ? "" : "Mat", trans == 'N' ? "" : "Transpose");
  ...
  PetscCallExternalVoid("scale", htool::scale(data->scale, X));
  PetscFunctionReturn(PETSC_SUCCESS);
}
...
static PetscErrorCode MatFactorNumeric_Htool(Mat F, Mat A, const MatFactorInfo *)
{
  ...
  PetscFunctionBegin;
  PetscCall(MatShellGetContext(A, &a));
  PetscCall(MatShellGetScalingShifts(A, (PetscScalar *)MAT_SHELL_NOT_ALLOWED, &scale, (Vec *)MAT_SHELL_NOT_ALLOWED, (Vec *)MAT_SHELL_NOT_ALLOWED, (Vec *)MAT_SHELL_NOT_ALLOWED, (Mat *)MAT_SHELL_NOT_ALLOWED, (IS *)MAT_SHELL_NOT_ALLOWED, (IS *)MAT_SHELL_NOT_ALLOWED));
