...
PetscErrorCode MatView_SeqAIJ(Mat A, PetscViewer viewer)
{
  ...
  if (isascii) PetscCall(MatView_SeqAIJ_ASCII(A, viewer));
...
static PetscErrorCode MatShift_SeqAIJ(Mat A, PetscScalar v)
{
  ...
  if (diagDense) {
    ...
    for (PetscInt i = 0; i < PetscMin(A->rmap->n, A->cmap->n); i++) a->imax[i] += mdiag[i];
...
PetscErrorCode MatSeqAIJGetArray(Mat A, PetscScalar *array[])
{
  ...
  PetscFunctionBegin;
  if (A->structure_only || aij->ops->getarray == NULL) *array = aij->a;
  else PetscCall((*aij->ops->getarray)(A, array));
  PetscFunctionReturn(PETSC_SUCCESS);
}
...
PetscErrorCode MatSeqAIJRestoreArray(Mat A, PetscScalar *array[])
{
  ...
  PetscFunctionBegin;
  if (A->structure_only || aij->ops->restorearray == NULL) *array = NULL;
  else PetscCall((*aij->ops->restorearray)(A, array));
  PetscCall(PetscObjectStateIncrease((PetscObject)A));
  PetscFunctionReturn(PETSC_SUCCESS);
}
...
PetscErrorCode MatSeqAIJGetArrayRead(Mat A, const PetscScalar *array[])
{
  ...
  PetscFunctionBegin;
  if (A->structure_only || aij->ops->getarrayread == NULL) *array = aij->a;
  else PetscCall((*aij->ops->getarrayread)(A, array));
  PetscFunctionReturn(PETSC_SUCCESS);
}
...
PetscErrorCode MatSeqAIJRestoreArrayRead(Mat A, const PetscScalar *array[])
{
  ...
  PetscFunctionBegin;
  if (A->structure_only || aij->ops->restorearrayread == NULL) *array = NULL;
  else PetscCall((*aij->ops->restorearrayread)(A, array));
  PetscFunctionReturn(PETSC_SUCCESS);
}
...
PetscErrorCode MatSeqAIJGetArrayWrite(Mat A, PetscScalar *array[])
{
  ...
  PetscFunctionBegin;
  if (A->structure_only || aij->ops->getarraywrite == NULL) *array = aij->a;
  else PetscCall((*aij->ops->getarraywrite)(A, array));
  PetscCall(PetscObjectStateIncrease((PetscObject)A));
  PetscFunctionReturn(PETSC_SUCCESS);
}
...
PetscErrorCode MatSeqAIJRestoreArrayWrite(Mat A, PetscScalar *array[])
{
  ...
  PetscFunctionBegin;
  if (A->structure_only || aij->ops->restorearraywrite == NULL) *array = NULL;
  else PetscCall((*aij->ops->restorearraywrite)(A, array));
  PetscFunctionReturn(PETSC_SUCCESS);
}
...
PetscErrorCode MatSetPreallocationCOO_SeqAIJ(Mat mat, PetscCount coo_n, PetscInt coo_i[], PetscInt coo_j[])
{
  ...
  PetscCheck(coo_n == 0 || i[coo_n - 1] < M, PETSC_COMM_SELF, PETSC_ERR_ARG_OUTOFRANGE, "COO row index %" PetscInt_FMT " is >= the matrix row size %" PetscInt_FMT, i[coo_n - 1], M);
...
#endif
#if PetscDefined(HAVE_VIENNACL) && PetscDefined(HAVE_VIENNACL_NO_CUDA)
