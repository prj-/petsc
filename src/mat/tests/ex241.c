...
int main(int argc, char **argv)
{
  ...
  PetscCall(MatHtoolGetMaxLeafSize(A, &maxleafsize));
  ...
  /* verify that MatGetDiagonalBlock() returns the proper matrix*/
  {
    ...
    for (PetscMPIInt i = 0; i < size; ++i) {
      ...
      if (i == rank) {
        ...
        PetscCheck(norm < PETSC_SMALL, PETSC_COMM_SELF, PETSC_ERR_PLIB, "Difference greater than PETSC_SMALL = %g", (double)norm);
